...
double distancePointSegment(Point p, Line l) {
  ...
  if (dot1 >= 0 && dot2 >= 0)
    return distancePointLine(p, l);
  else
    return min(p.distanceTo(l.p), p.distanceTo(l.q));
}
...
Point closestPointInSegment(Point p, Line s) {
  ...
  if (u * w < 0 || u * w > s.p.distanceTo(s.q)) {
    if (p.distanceTo(s.p) < p.distanceTo(s.q))
      return s.p;
    else
      return s.q;
  } else
    return Point(s.p.x + res.x, s.p.y + res.y);
}
  ...
  if (abs(s1.v ^ s2.v) <= EPS) {
    ...
    if (s1.p.x == s1.q.x && s2.p.x == s2.q.x && s1.p.x == s2.p.x) {
      ...
      if (ansl.x == ansr.x && ansl.y == ansr.y) {
        ...
        // cout << ansl.x << " " << ansl.y << endl << ansr.x << " " << ansr.y <<
        // endl;
...
double distanceSegmentSegment(Line l1, Line l2) {
  ...
  if ((l1.v ^ l2.v) != 0) {
    ...
    bool ok1 = (cross1 <= 0 && cross2 >= 0) ||
               (distancePointLine(r1.p, r3) > distancePointLine(r1.q, r3));

    cross1 = (Point(r1.p, r3.p) ^ r1.v);
    cross2 = (Point(r1.p, r3.q) ^ r1.v);
    if (cross2 < cross1) swap(cross1, cross2);

    bool ok2 = (cross1 <= 0 && cross2 >= 0) ||
               (distancePointLine(r3.p, r1) > distancePointLine(r3.q, r1));

    cross1 = (Point(r3.p, r2.p) ^ r3.v);
    cross2 = (Point(r3.p, r2.q) ^ r3.v);
    if (cross2 < cross1) swap(cross1, cross2);

    bool ok3 = (cross1 <= 0 && cross2 >= 0) ||
               (distancePointLine(r2.p, r3) > distancePointLine(r2.q, r3));

    cross1 = (Point(r2.p, r3.p) ^ r2.v);
    cross2 = (Point(r2.p, r3.q) ^ r2.v);
    if (cross2 < cross1) swap(cross1, cross2);

    bool ok4 = (cross1 <= 0 && cross2 >= 0) ||
               (distancePointLine(r3.p, r2) > distancePointLine(r3.q, r2));

    cross1 = (Point(r4.p, r1.p) ^ r4.v);
    cross2 = (Point(r4.p, r1.q) ^ r4.v);
    if (cross2 < cross1) swap(cross1, cross2);

    bool ok5 = (cross1 <= 0 && cross2 >= 0) ||
               (distancePointLine(r1.p, r4) > distancePointLine(r1.q, r4));

    cross1 = (Point(r1.p, r4.p) ^ r1.v);
    cross2 = (Point(r1.p, r4.q) ^ r1.v);
    if (cross2 < cross1) swap(cross1, cross2);

    bool ok6 = (cross1 <= 0 && cross2 >= 0) ||
               (distancePointLine(r4.p, r1) > distancePointLine(r4.q, r1));

    cross1 = (Point(r4.p, r2.p) ^ r4.v);
    cross2 = (Point(r4.p, r2.q) ^ r4.v);
    if (cross2 < cross1) swap(cross1, cross2);

    bool ok7 = (cross1 <= 0 && cross2 >= 0) ||
               (distancePointLine(r2.p, r4) > distancePointLine(r2.q, r4));

    cross1 = (Point(r2.p, r4.p) ^ r2.v);
    cross2 = (Point(r2.p, r4.q) ^ r2.v);
    if (cross2 < cross1) swap(cross1, cross2);

    bool ok8 = (cross1 <= 0 && cross2 >= 0) ||
               (distancePointLine(r4.p, r2) > distancePointLine(r4.q, r2));
...
double distanceSegmentRay(Line s, Line r) {
  if ((s.v ^ r.v) != 0) {
    ...
    bool ok1 = (cross1 <= 0 && cross2 >= 0) ||
               (distancePointLine(r1.p, r) > distancePointLine(r1.q, r));
    ...
    bool ok2 = (cross1 <= 0 && cross2 >= 0) ||
               (distancePointLine(r.p, r1) > distancePointLine(r.q, r1));
    ...
    bool ok3 = (cross1 <= 0 && cross2 >= 0) ||
               (distancePointLine(r2.p, r) > distancePointLine(r2.q, r));
    ...
    bool ok4 = (cross1 <= 0 && cross2 >= 0) ||
               (distancePointLine(r.p, r2) > distancePointLine(r.q, r2));
  ...
  if (dot >= 0)
    ans = min(ans, distancePointLine(r.p, s));
  else
    ans = min(ans, min(r.p.distanceTo(s.p), r.p.distanceTo(s.q)));

  dot = Point(r.p, s.p) * r.v;
  if (dot >= 0)
    ans = min(ans, distancePointLine(s.p, r));
  else
    ans = min(ans, r.p.distanceTo(s.p));

  dot = Point(r.p, s.q) * r.v;
  if (dot >= 0)
    ans = min(ans, distancePointLine(s.q, r));
  else
    ans = min(ans, r.p.distanceTo(s.q));
...
double distanceSegmentLine(Line s, Line l) {
  ...
  if (cross1 <= 0 && cross2 >= 0)
    ...
    return min(distancePointLine(s.p, l), distancePointLine(s.q, l));
}
...
double distanceLineRay(Line l, Line r) {
  ...
  if (cross2 < cross1) swap(cross1, cross2);
  if ((cross1 <= 0 && cross2 >= 0) ||
      (distancePointLine(r.p, l) > distancePointLine(r.q, l)))
    return 0;
  return distancePointLine(r.p, l);
}
...
double distanceRayRay(Line r1, Line r2) {
  if ((r1.v ^ r2.v) != 0) {
    ...
    if (cross2 < cross1) swap(cross1, cross2);
    bool ok1 = (cross1 <= 0 && cross2 >= 0) ||
               (distancePointLine(r2.p, r1) > distancePointLine(r2.q, r1));
    ...
    bool ok2 = (cross1 <= 0 && cross2 >= 0) ||
               (distancePointLine(r1.p, r2) > distancePointLine(r1.q, r2));
  ...
  if (dot >= 0)
    ans = min(ans, distancePointLine(r1.p, r2));
  else
    ans = min(ans, r2.p.distanceTo(r1.p));

  dot = Point(r1.p, r2.p) * r1.v;
  if (dot >= 0)
    ans = min(ans, distancePointLine(r2.p, r1));
  else
    ans = min(ans, r1.p.distanceTo(r2.p));
...
double circleCircleIntersection(Circle c1, Circle c2) {
  ...
  double x1 = c1.c.x, x2 = c2.c.x, y1 = c1.c.y, y2 = c2.c.y, r1 = c1.r,
         r2 = c2.r;
...
vector<Point> intersectionLineCircle(Line l, Circle circ) {
  // NOT TESTED!!!!!!!!
  // no intersection
  if ((l.c * l.c) / (circ.r * circ.r) > l.a * l.a + l.b * l.b + EPS)
    return vector<Point>();

  double x0 = -l.a * l.c / (l.a * l.a + l.b * l.b),
         y0 = -l.b * l.c / (l.a * l.a + l.b * l.b);
  // one intersection
...
vector<Point> intersectionCircleCircle(Circle c1, Circle c2) {
  ...
  if (c1.c == c2.c) {
    // if radius are equal = infinite intersections(return 3 points to
...
Point barycenter(Point &a, Point &b, Point &c, double pA, double pB,
                 double pC) {
...
Point circumcenter(Point &a, Point &b, Point &c) {
  double pA = Point(b, c).squareSize(), pB = Point(a, c).squareSize(),
         pC = Point(a, b).squareSize();
  return barycenter(a, b, c, pA * (pB + pC - pA), pB * (pC + pA - pB),
                    pC * (pA + pB - pC));
}
...
Point incenter(Point &a, Point &b, Point &c) {
  return barycenter(a, b, c, Point(b, c).size(), Point(a, c).size(),
                    Point(a, b).size());
}
...
Point orthocenter(Point &a, Point &b, Point &c) {
  double pA = Point(b, c).squareSize(), pB = Point(a, c).squareSize(),
         pC = Point(a, b).squareSize();
  return barycenter(a, b, c, (pA + pB - pC) * (pC + pA - pB),
                    (pB + pC - pA) * (pA + pB - pC),
                    (pC + pA - pB) * (pB + pC - pA));
}
  ...
  for (int i = 0; i < v.size(); i++) {
    if (!circ.inside(v[i])) {
      ...
      for (int j = 0; j < i; j++) {
        if (!circ.inside(v[j])) {
          circ =
              Circle((v[i] + v[j]).scale(0.5), Point(v[i], v[j]).size() * 0.5);
          for (int k = 0; k < j; k++) {
            if (!circ.inside(v[k])) {

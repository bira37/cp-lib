...
struct HLD {
  ...
  struct SegmentTree{
    ...
    void propagate(int cur, int l, int r){
      ...
      st[cur].val = st[cur].lazy;
      
      if(l != r){
        st[2*cur].lazy = st[cur].lazy;
        st[2*cur+1].lazy = st[cur].lazy;
      }
  ...
  node path_query(int a, int b, bool edge_query = false){
    ...
    if(L[b] < L[a]) swap(a,b);
    if(in[a]+edge_query <= in[b]) ans = ans.merge(st.query(1, 1, t, in[a]+edge_query, in[b]));
    return ans;
  }
...
int32_t main(){
  ...
  for(int i=2; i<=n; i++){
    ...
    adj[u].pb(v);
    adj[v].pb(u);
    edge.pb(ii(u,v)); cost.pb(c);
  }
  HLD hld(n);
  hld.calculate();
  for(int i=0; i<edge.size(); i++){
    if(hld.L[edge[i].ff] > hld.L[edge[i].ss]) v[hld.in[edge[i].ff]] = cost[i];
    else v[hld.in[edge[i].ss]] = cost[i];
  }
  hld.build_ds();
  int q;
  cin >> q;
  while(q--){
    ...
    int u, v;
    cin >> op >> u >> v;
    if(op == 1) cout << (hld.path_query(u, v, true).val%2 ? "WE NEED BLACK PANDA" : "JAKANDA FOREVER") << endl;
    else {
      u--;
      hld.edge_update(edge[u].ff, edge[u].ss, v);
    }
  }  
}

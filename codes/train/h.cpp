...
int32_t main(){
  ...
  while(T--){
    ...
    for(int i = 0; i < pol.size(); i++) {
      areat += (pol[i]^pol[(i+1)%n]); 
    }
    areat /= 2.;
    
    Geo2D::Point pr((pol[0].x+pol[1].x)/2, (pol[0].y+pol[1].y)/2);
    Geo2D::Point pr1(pr,pol[1]);
    pr1.rotate(PI/4);
    Geo2D::Point pr2(pr,pol[1]);
    pr2.rotate(3*PI/4);
    
    
    int l=1,r=0;
    Geo2D::Point interl = pr;
    
    Geo2D::Line lr(pr, pr+pr1);
    for(int i=1; i < n; i++) {
      Geo2D::Line ls(pol[i],pol[(i+1)%n]);
      double dis = Geo2D::distanceSegmentRay(ls,lr);
      if(dis<=EPS){
        double a1 = ls.q.y - ls.p.y;
        double b1 = ls.p.x - ls.q.x;
        double c1 = a1*ls.p.x + b1*ls.p.y;
        double a2 = lr.q.y - lr.p.y;
        double b2 = lr.p.x - lr.q.x;
        double c2 = a2*lr.p.x + b2*lr.p.y;
        double det = a1*b2 - a2*b1;
        double x = (double)(b2*c1 - b1*c2)/(double)det*1.;
        double y = (double)(a1*c2 - a2*c1)/(double)det*1.;
        interl=Geo2D::Point(x,y);
        
        l = i+1;
      }
    }
    
    Geo2D::Point interr = pr;
    
    lr=Geo2D::Line (pr, pr+pr2);
    for(int i=1; i < n; i++) {
      Geo2D::Line ls(pol[i],pol[(i+1)%n]);
      double dis = Geo2D::distanceSegmentRay(ls,lr);
      //cout << ls.p.x << " " << ls.p.y << " " << ls.q.x << " "  << ls.q.y << endl;
      //cout << lr.p.x << " " << lr.p.y << " " << lr.q.x << " " << lr.q.y << endl;
      //cout << i << " " << dis << endl;
      //cout <<endl;
      if(dis<=EPS){
        double a1 = ls.q.y - ls.p.y;
        ...
        double x = (double)(b2*c1 - b1*c2)/(double)det*1.;
        double y = (double)(a1*c2 - a2*c1)/(double)det*1.;
        interr=Geo2D::Point(x,y);
        
        r = i;
      }
    }
    vector< Geo2D::Point > npol;
    npol.pb(pr);
    npol.pb(interl);
    ...
    for(int i = 0; i < npol.size(); i++) {
      area += (npol[i]^npol[(i+1)%(int)npol.size()]); 
    }
    area /= 2.;
    cout << fixed  << setprecision(10) << area/areat << endl;
  }
}
